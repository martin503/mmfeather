...
class DeepSeekV3Tokenizer:
    def __init__(self, tokenizer_path: PathType):
        super().__init__()
        self.tokenizer_path = tokenizer_path
        self.tokenizer = self._load_tokenizer_from_path(tokenizer_path)
        self.config = self._load_config(os.path.join(tokenizer_path, "tokenizer_config.json"))
    ...
    def _load_config(self, config_path: PathType) -> dict[str, Any]:
        with open(config_path, "r") as f:
            return json.load(f)

    def _load_tokenizer_from_path(self, tokenizer_path: PathType) -> Tokenizer:
